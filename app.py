...
def format_time_12hr(time_24):
    if not time_24: return ""
    try: return datetime.strptime(time_24, "%H:%M:%S").strftime("%I:%M:%S %p")
    except: return time_24

def get_db():
    db = getattr(g, '_database', None)
    if db is None:
        db = g._database = sqlite3.connect(DATABASE, timeout=10.0) # AYOS: May timeout na
...
def init_db():
    with app.app_context():
        ...
        if not cur.fetchone(): db.execute("INSERT INTO users (username, password, role, status) VALUES ('admin', 'admin123', 'Admin', 'approved')")
        db.commit()
...
def login():
    if request.method == 'POST':
        ...
        if user:
            if user['status']!= 'approved': flash('Your account is pending approval by Admin', 'warning'); return redirect(url_for('login'))
...
def register_user():
    if request.method == 'POST':
        ...
        try: db.execute("INSERT INTO users(username, password, role, status) VALUES(?,?,?, 'pending')", (username, password, role)); db.commit(); flash('Registration Sent! Wait for Admin approval.', 'success'); return redirect(url_for('login'))
...
@app.route('/approvals')
def approvals():
    if not session.get('logged_in') or session['role']!= 'Admin': flash('Admin access only', 'danger'); return redirect(url_for('login'))
...
def dashboard():
    ...
    today_str = date.today().isoformat(); db = get_db()
    ...
    return render_template('dashboard.html', total=total, present=present_today, late=late_today, absent=absent_today, recent=recent, school=SCHOOL_NAME, grade=GRADE_LEVEL, today=date.today(), format_time=format_time_12hr)

@app.route('/students')
def students():
    if not session.get('logged_in') or session['role']!= 'Admin': return redirect(url_for('login'))
...
def register_teacher():
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        try: db.execute("INSERT INTO teachers(teacher_id, name, subject, contact) VALUES(?,?,?,?)", (teacher_id, name, subject, contact)); db.commit(); flash('Teacher Registered Successfully', 'success'); return redirect(url_for('teachers'))
        except sqlite3.IntegrityError: flash('Error: Teacher ID already exists', 'danger')
    return render_template('register_teacher.html', school=SCHOOL_NAME, grade=GRADE_LEVEL)

@app.route('/edit_teacher/<int:id>', methods=['GET', 'POST'])
def edit_teacher(id):
    if not session.get('logged_in') or session['role']!= 'Admin': flash('Unauthorized! Admin access only.', 'danger'); return redirect(url_for('dashboard'))
    db = get_db()
    teacher = db.execute("SELECT * FROM teachers WHERE id =?", (id,)).fetchone()
    if request.method == 'POST':
        teacher_id = request.form['teacher_id']; name = request.form['name']; subject = request.form['subject']; contact = request.form['contact']
        try: db.execute("UPDATE teachers SET teacher_id=?, name=?, subject=?, contact=? WHERE id=?", (teacher_id, name, subject, contact, id)); db.commit(); flash(f'Teacher {name} updated successfully.', 'success'); return redirect(url_for('teachers'))
        except sqlite3.IntegrityError: flash('Error: Teacher ID already exists', 'danger')
...
def delete_teacher(id):
    if not session.get('logged_in') or session['role']!= 'Admin': flash('Unauthorized! Admin access only.', 'danger'); return redirect(url_for('dashboard'))
    db = get_db()
    try: db.execute("DELETE FROM teachers WHERE id =?", (id,)); db.commit(); flash('Teacher deleted successfully.', 'success')
    except Exception as e: db.rollback(); flash(f'Error deleting teacher: {e}', 'danger')
...
def register_student():
    ...
    if request.method == 'POST':
        ...
        db = get_db()
        try: db.execute("INSERT INTO students(student_id, name, grade_section, parent_name, parent_contact, qr_code_path) VALUES(?,?,?,?,?,?)", (student_id, name, section, pname, pcontact, qr_filename)); db.commit(); flash('Student Registered Successfully', 'success'); qr_path = qr_filename; student_name = name
        except sqlite3.IntegrityError: flash('Error: Student ID already exists', 'danger')
...
def scan():
    ...
    now = datetime.now(); today_str = date.today().isoformat(); cur_time_24 = now.strftime("%H:%M:%S"); cur_time_12 = now.strftime("%I:%M:%S %p"); db = get_db()
    student = db.execute("SELECT * FROM students WHERE student_id=?", [student_id]).fetchone()
    if not student: return jsonify({'status': 'error', 'message': 'Student not found'})
    ...
    if scan_type == 'in':
        if record and record['time_in']: return jsonify({'status': 'error', 'message': f'{student["name"]} already TIMED IN at {format_time_12hr(record["time_in"])}'})
        if not record: db.execute("INSERT INTO attendance(student_id, date, time_in, status, scanned_by) VALUES(?,?,?,?,?)", (student_id, today_str, cur_time_24, status, session['username']))
        else: db.execute("UPDATE attendance SET time_in=?, status=? WHERE id=?", (cur_time_24, status, record['id']))
        message = f"{student['name']} TIME IN: {cur_time_12} - {status}"
    elif scan_type == 'out':
        if not record or not record['time_in']: return jsonify({'status': 'error', 'message': f'{student["name"]} has no TIME IN record yet'})
        if record['time_out']: return jsonify({'status': 'error', 'message': f'{student["name"]} already TIMED OUT at {format_time_12hr(record["time_out"])}'})
        db.execute("UPDATE attendance SET time_out=? WHERE id=?", (cur_time_24, record['id'])); message = f"{student['name']} TIME OUT: {cur_time_12}"

    db.commit()

    sms_service.send_sms(student['parent_contact'], f"{SCHOOL_NAME}: {message}. Thank you.") # AYOS: Huli na yung SMS
...
def attendance():
    if not session.get('logged_in'): return redirect(url_for('login'))
    db = get_db(); records = db.execute("SELECT a.*, s.name FROM attendance a JOIN students s ON a.student_id=s.student_id ORDER BY a.date DESC").fetchall()
    return render_template('attendance.html', records=records, school=SCHOOL_NAME, grade=GRADE_LEVEL, format_time=format_time_12hr)
...
def reports():
    ...
    late_today = db.execute("SELECT COUNT(*) FROM attendance WHERE date=? AND status='Late'", [today_str]).fetchone()[0]
    absent_today = 0 if present_today == 0 else total - present_today
    return render_template('reports.html', total=total, present=present_today, late=late_today, absent=absent_today, school=SCHOOL_NAME, grade=GRADE_LEVEL, today=date.today())

@app.route('/reset_attendance', methods=['POST'])
def reset_attendance():
    if not session.get('logged_in') or session['role']!= 'Admin': flash('Unauthorized Access', 'danger'); return redirect(url_for('login'))
